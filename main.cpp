...
bool backtrack_help(NFA<T> nfa, Str str, T qi) {
    std::vector<T> epsVect = nfa.d(qi, Character("\u03B5"));
    ...
    if (!str.isEmpty()) {
        ...
        std::vector<T> charVect = nfa.d(qi, ch);
        str.eraseFront();
...
void runTestCases() {
    ...
    {  // DFAs Initialized Below, Test Calls At The Bottom
        ...
        // COMMENTED OUT DURING CHECKPOINT 3 & 4 (NFA TESTING)
        ...
        { // Task 21 & 22 - Equality Tests & Verifying Union, Intersect, Complement
    ...
    {   // NFAs
        {  // Tasks 25+
            ...
            { // Task 32 - Backtracking Tests
                ...
                cout << "\nTest[12] \t : \t" << backtracking(nfa6, Str("0"));
            }
